...
(function(){
  ...
  function docCss(){return '@page{size:A4 landscape;margin:7mm}*{box-sizing:border-box}html,body{margin:0!important;padding:0!important;background:#fff;color:#111;font-family:Arial,Tahoma,sans-serif;font-size:9.4px;width:297mm;min-width:297mm;max-width:297mm;overflow:visible!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.sheet{direction:rtl;width:283mm;max-width:283mm;min-height:190mm;margin:0 auto!important;padding:0!important;overflow:visible}.no-print{position:fixed;top:8px;left:8px;z-index:999;display:flex;gap:7px}.no-print button{font-weight:900;border:2px solid #000;border-radius:9px;background:#fff;color:#000;padding:8px 12px}.hp-doc-head{direction:ltr;display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2.4px solid #0b2442;padding-bottom:4px;margin:0 0 7px;width:100%}.brand{display:flex;align-items:center;gap:8px;max-width:45%}.brand.r{direction:rtl;text-align:right}.brand.l{direction:ltr;text-align:left}.brand img{width:48px;height:48px;object-fit:contain;flex:0 0 auto}.title-en{font-size:19px;font-weight:900;color:#0b2442;line-height:1.05}.title-ar{font-size:21px;font-weight:900;color:#ad7b25;line-height:1.05}.sub{font-size:9px;font-weight:900;color:#111;margin-top:3px;white-space:nowrap}.doc-title{text-align:center;font-size:17px;font-weight:900;margin:4px 0 7px;text-decoration:underline}.doc-chip{display:none!important}.meta{display:grid;grid-template-columns:repeat(3,1fr);gap:3px 10px;margin-bottom:6px;width:100%}.meta div{border-bottom:1px dotted #777;padding:2px 0;display:flex;justify-content:space-between;gap:8px;min-width:0}.meta b{white-space:nowrap}.meta span{overflow-wrap:anywhere}table{width:100%;border-collapse:collapse;table-layout:fixed;page-break-inside:auto}thead{display:table-header-group}tr{page-break-inside:avoid;break-inside:avoid}th,td{border:1px solid #000;text-align:center;vertical-align:middle;padding:3px 2px;font-size:8.8px;line-height:1.25;white-space:normal;word-break:break-word;overflow-wrap:anywhere}th{background:#e7edf8;font-weight:900}.totals{width:72mm;max-width:72mm;margin-top:7px;margin-right:auto;table-layout:auto}.totals td{font-weight:900;font-size:9.2px}.terms,.note{margin-top:8px;line-height:1.55;font-weight:900;text-align:right;font-size:9.2px}.sign,.foot{display:none!important}@media screen{body{background:#f3f4f6}.sheet{background:#fff;margin:10px auto!important;padding:7mm!important;box-shadow:0 0 0 1px #ddd}}@media print{html,body{width:297mm!important;min-width:297mm!important;max-width:297mm!important}.no-print{display:none!important}.sheet{width:283mm!important;max-width:283mm!important;margin:0 auto!important;padding:0!important;box-shadow:none!important}.sign,.foot{display:none!important}}'}
  ...
  function openHtml(html,autoPrint){var w=window.open('','_blank','width=1400,height=900,scrollbars=yes,resizable=yes'); if(!w){toast('المتصفح منع فتح المستند. اسمح بالـ Popups.'); return false} w.document.open(); w.document.write(html+(autoPrint?'<script>setTimeout(function(){try{document.title=document.title||\'Haydar Pack\';window.focus();window.print()}catch(e){}},650);<\/script>':'')); w.document.close(); return true}
  ...
  function sanitizeStoredHtml(html,rec){
    ...
    html=html.replace(/<style>[\s\S]*?<\/style>/i,'<style>'+docCss()+'</style>');
  ...
  function boot(){if(booted)return;booted=true;try{ensureStore();injectStyle();wrapReports();setTimeout(renderCenter,400);setTimeout(renderCenter,1200); console.log('Haydar Pack V57.5 Print Layout Repair loaded',VERSION)}catch(e){log('V54_BOOT_ERROR',String(e&&e.message||e),'boot')}}
...
(function(){
  ...
  function html(){
    ...
    var c=s.counts;
    return '<h3>V57.5 Legacy Clean Gate — فحص رأس المال والسيولة</h3>'+
  ...
  function boot(){try{bind();wrapShowPage();place();setTimeout(place,800);setInterval(function(){if((window.activePage||'')==='sync'||document.querySelector('#dr-sync.open,#dr-sync .drawer'))place()},2500);console.log('Haydar Pack V57.5 Legacy Clean Gate loaded',VERSION)}catch(e){try{console.error(e)}catch(_){}}}
...
/* ===== V57.5 Internal Legacy Cleanup + Smart Summary (no new JS files) ===== */
(function(){
  'use strict';
  var VERSION='57.5.0-print-layout-repair', SITE_VERSION='57_5printfix';
