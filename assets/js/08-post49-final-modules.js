...
(function(){
  ...
  function docHtml(title,no,status,metaHtml,head,body,totals,extra){return '<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>'+esc(no+' - '+title)+'</title><style>'+docCss()+'</style></head><body><div class="no-print"><button onclick="window.print()">طباعة / PDF</button><button onclick="window.close()">إغلاق</button></div><div class="sheet">'+header()+'<div class="doc-title">'+esc(title)+' <span class="doc-chip">'+esc(status||STATUS.draft)+'</span></div>'+metaHtml+'<table><thead>'+head+'</thead><tbody>'+body+'</tbody></table>'+totals+(extra||'')+'<div class="sign"><div>توقيع العميل</div><div>المبيعات</div><div>الإدارة</div></div></div><div class="foot"><span>Haydar Pack</span><span>'+esc(no)+'</span><span>'+esc(today())+'</span></div></body></html>'}
